...
const renderGuidance = (rule) => {
  const details = GUIDANCE[rule];
  if (!details) return null;

  return (
    <div className="muted small">
      <div>
        <strong>Potential misuse:</strong> {details.risk}
      </div>
      <div>
        <strong>Why hide:</strong> {details.hide}
      </div>
    </div>
  );
};
          ...
          <div className="result-card" key={result.url}>
            ...
            <div className="result-meta">
              <span className={`badge ${severity}`}>
                {totalFindings ? `${totalFindings} findings` : "No findings"}
              </span>
            ...
            {result.findings.length ? (
              result.findings.map((item) => (
                <div className="result-list" key={`${item.rule}-${item.source}`}>
                  <strong>{item.rule}</strong>
                  <div className="muted small">
                    {item.total} matches · {item.source}
                  </div>
                  {renderGuidance(item.rule)}
              ...
              <div className="muted">No secrets detected in accessible HTML.</div>
            )}
